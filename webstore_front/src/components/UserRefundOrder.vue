<template>
	<div>
		...
		<el-table :data="pagedOrders" style="width: 100%">
			...
			<el-table-column label="操作">
				<template slot-scope="scope">
					<el-button type="warning" size="small" @click="showShipDialog(scope.row)">撤回</el-button>
				</template>
			</el-table-column>
		</el-table>
		...
		<el-dialog :visible.sync="shipDialogVisible" title="撤销退货" @close="resetShipDialog">
			<p>确定要撤销退货吗？</p>
	...
	export default {
		...
		methods: {
			getOrder() {
				...
				axios.get('http://localhost:8080/Goods/userFindRefund', {
					params: {
						userId: userId
					}
				}).then(response => {
					...
					let orderArray = response.data.data.map(item => ({
						...
						status: item.goods_status == 5 ? '待处理' : '已发货'
					}));
			...
			shipOrder() {
				...
				axios.post('http://localhost:8080/orders/undoGoods', {
